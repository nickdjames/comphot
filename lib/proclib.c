		...
		for (tgt_x = 0; tgt_x < axes[0]*drizzle; tgt_x++) { // loop through the target pixels
			...
			for (image = 0; image < n; image++) {
				if (shiftlist)
					...
					shift.x = shift.y = 0;	
				src_x = tgt_x + stepsize * floor(drizzle*shift.x/stepsize+0.5);
				src_y = tgt_y + stepsize * floor(drizzle*shift.y/stepsize+0.5);
...
float *shift_and_subtract(float *im1, float *im2, long axes[2], float sky, COORD shift)
{
	...
	for (y = 0; y< axes[1]; y++) {
		for (x = 0; x < axes[0]; x++) {
			sub_x = x + (long) floor(shift.x+0.5);
			sub_y = y + (long) floor(shift.y+0.5);
